...
;;Attribute to Professor Adam Shaw
...
(define (moves-king game loc)
  (match game
    [(ChessGame b p _ (Castles 0-0 0-7 7-0 7-7))
     (match (board-ref b loc)
       [(Some (Piece (not 'King) _)) (error "not king")]
       ['None (error "empty square")]
       [_ (append (move-if-possible b loc -1  0)
                  (move-if-possible b loc -1  1)
                  (move-if-possible b loc  0  1)
                  (move-if-possible b loc  1  1)
                  (move-if-possible b loc  1  0)
                  (move-if-possible b loc  1 -1)
                  (move-if-possible b loc  0 -1)
                  (move-if-possible b loc -1 -1)
                  (if
                   ...
                   (cond
                     [(and 7-0 7-7)
                      (append
                       (if (legal-castle?
                            game
                            (CastleMove (Loc 7 4) (Loc 7 1)
                                        (Loc 7 0) (Loc 7 2) 'White))
                           (list (CastleMove (Loc 7 4) (Loc 7 1)
                                             (Loc 7 0) (Loc 7 2) 'White))
                           '())
                       (if (legal-castle?
                            game
                            (CastleMove (Loc 7 4) (Loc 7 6)
                                        (Loc 7 7) (Loc 7 5) 'White))
                           (list (CastleMove (Loc 7 4) (Loc 7 6)
                                             (Loc 7 7) (Loc 7 5) 'White))
                           '()))]
                     [7-0 (if (legal-castle?
                               game
...
(check-expect (moves-king new-game (Loc 7 4)) '())
(check-expect
 (sort-moves
  (moves-king
   (ChessGame
     ...
     (match (board-ref b loc)
       ...
       [(Some (Piece 'Pawn player))
        (local
          ...
          (match loc
            [(Loc 0 _) (error "pawn cannot be in rank 0")]
            [(Loc 7 _) (error "pawn cannot be in rank 7")]
            [(Loc rank col)
             (match
               ...
               [moves
                (if
                 (or
                  (legal-promo? g (PromoMove loc (val-of (+loc loc δ-rank 0))
                  ...
                  (and (not (none? (+loc loc δ-rank -1)))
                       (legal-promo?
                        ...
                        (PromoMove loc (val-of (+loc loc δ-rank -1))
                                   (Piece 'Pawn p)
                                   (Some (Piece 'Bishop (opponent p))) 'Queen)))
                  (and (not (none? (+loc loc δ-rank 1)))
                       (legal-promo?
                        g
                        (PromoMove loc (val-of (+loc loc δ-rank 1))
...
;;the original reference ignores the situation in which two kings adjacent
;;test if a king can be attacked by the opponent king
                 ...
                 (match m
                   ...
                   [(PromoMove src dst moved cap promoto)
                    (and
                     (match cap
                       ['None #t]
                       [(Some (Piece _ pl)) (not (equal? pl p))]
                       [_ #t])
                     (not (in-check?
                           (ChessGame
                            (board-update
...
(: enP? : ChessGame Move -> Boolean)
;; test if a move is en passant
(define (enP? g m)
  (match* (g m)
    [((ChessGame b p his _) (StdMove src dst piece cap))
     (match* (his dst)
       [((cons m1 _) (Loc dR dC))
        (match* (m1 src)
          [((StdMove m1s m1d m1p _) (Loc mC mR))
           (match* (m1s m1d)
             [((Loc m1sR m1sC) (Loc m1dR m1dC))
              (and (= m1sC m1dC)
                   (= (abs (- m1sR m1dR)) 2)
                   (or (equal? m1p (Piece 'Pawn 'White))
                       (equal? m1p (Piece 'Pawn 'Black)))
                   (or (equal? cap (Some (Piece 'Pawn 'White)))
                       (equal? cap (Some (Piece 'Pawn 'Black))))
                   (or (equal? piece (Piece 'Pawn 'White))
                       (equal? piece (Piece 'Pawn 'Black)))
                   (= (abs (- dR m1dR)) 1)
                   (= dC m1dC))])]
          [(_ _) #f])]
       [('() _) #f])]
...
(: legal-castle? : ChessGame Move -> Boolean)
(define (legal-castle? game move)
  (match* (game move)
    [((ChessGame board turn _ (Castles 0-0 0-7 7-0 7-7))
      (CastleMove king-src king-dst rook-src rook-dst player))
     (match* ((board-ref board king-src)
              (board-ref board king-dst)
              (board-ref board rook-src)
              (board-ref board rook-dst))
       [((Some (Piece 'King turn))
         'None
         (Some (Piece 'Rook turn))
         'None)
        (and
         (not (in-check? game))
         (not (in-check?
               (ChessGame
                (board-update
                 (board-update
                  (board-update
                   (board-update board king-src 'None)
                   king-dst
                   (Some (Piece 'King turn)))
                  rook-src 'None)
                 rook-dst
            ...
            (or (and
                 ...
                 (match* (king-dst rook-src rook-dst)
                   [((Loc 0 6) (Loc 0 7) (Loc 0 5))
                    (match* ((board-ref board (Loc 0 5))
                             (board-ref board (Loc 0 6)))
                      [('None 'None) #t]
                      [(_ _) #f])]
                   [(_ _ _) #f]))
                (and
                 ...
                 (match* (king-dst rook-src rook-dst)
                   [((Loc 0 1) (Loc 0 0) (Loc 0 2))
                    (match* ((board-ref board (Loc 0 1))
                             (board-ref board (Loc 0 2))
                             (board-ref board (Loc 0 3)))
                      [('None 'None 'None) #t]
                      [(_ _ _) #f])]
                   [(_ _ _) #f])))]
           [(Loc 7 4)
            (or (and
                 ...
                 (match* (king-dst rook-src rook-dst)
                   [((Loc 7 6) (Loc 7 7) (Loc 7 5))
                    (match* ((board-ref board (Loc 7 5))
                             (board-ref board (Loc 7 6)))
                      [('None 'None) #t]
                      [(_ _) #f])]
                   [(_ _ _) #f]))
                (and
                 ...
                 (match* (king-dst rook-src rook-dst)
                   [((Loc 7 1) (Loc 7 0) (Loc 7 2))
                    (match* ((board-ref board (Loc 7 1))
                             (board-ref board (Loc 7 2))
                             (board-ref board (Loc 7 3)))
                      [('None 'None 'None) #t]
                      [(_ _ _) #f])]
                   [(_ _ _) #f])))]))]
       [(_ _ _ _) #f])]
...
(: legal-promo? : ChessGame Move -> Boolean)
(define (legal-promo? game move)
  (match* (game move)
    [((ChessGame board player _ _)
      (PromoMove src dst piece capture promo-to))
     (if (equal? piece (Piece 'Pawn 'White))
         (match* (src dst)
           [((Loc r1 c1) (Loc r2 c2))
            (and
             (= r1 1)
             (= r2 0)
             (if (equal? capture 'None)
                 (and (= c1 c2)
                      (equal? (board-ref board dst) 'None))
                 (and (or (= c1 (add1 c2))
                          (= c1 (sub1 c2)))
                      (not (equal? 'None (board-ref board dst))))))])
         (match* (src dst)
           [((Loc r1 c1) (Loc r2 c2))
            (and
             (= r1 6)
             ...
             (if (equal? capture 'None)
                 (and (= c1 c2)
                      (equal? 'None (board-ref board dst)))
                 (and (or (= c1 (add1 c2))
                          (= c1 (sub1 c2)))
                      (not (equal? 'None (board-ref board dst))))))]))]
    [(_ _) #f]))
     ...
     (match m
       [(StdMove src dst piece capt)
        (and (piece-at? b piece (Some src))
             ...
             (match capt
               ['None (unocc? b dst)]
               [(Some c) (or (enP? g m) (piece-at? b c (Some dst)))])
             (not (in-check? (relocate-piece g src dst))))]
       [(CastleMove _ _ _ _ _) (legal-castle? g m)]
       [(PromoMove _ _ _ _ _) (legal-promo? g m)])]))
           ...
           [(StdMove src dst pc _)
            (if (enP? g m)
                (ChessGame
                 ...
                 (cons m hist)
                 cas)
                (ChessGame (board-update (board-update b src 'None) dst
                                         (Some pc))
                           (opponent pl)
                           (cons m hist)
           ...
           [(CastleMove king-src king-dst rook-src rook-dst moved)
            (ChessGame
             (board-update
              (board-update
               (board-update
                (board-update b rook-dst (Some (Piece 'Rook moved)))
                rook-src 'None)
             ...
             (cons m hist)
             (match cas
               [(Castles 0-0 0-7 7-0 7-7)
                (match king-src
                  [(Loc 0 4) (Castles #f #f 7-0 7-7)]
                  [(Loc 7 4) (Castles 0-0 0-7 #f #f)])]))]
           [(PromoMove src dst moved cap promo)
            (ChessGame
             (board-update
              (board-update b dst (Some (Piece promo pl)))
              src 'None)
             (opponent pl)
             (cons m hist)
             (match cas
               [(Castles 0-0 0-7 7-0 7-7)
                (cond
                  [(equal? cap (Some (Piece 'Rook (opponent pl))))
                   (match dst
                     [(Loc 0 0) (Castles #f 0-7 7-0 7-7)]
                     [(Loc 0 7) (Castles 0-0 #f 7-0 7-7)]
                     [(Loc 7 0) (Castles 0-0 0-7 #f 7-7)]
                     [(Loc 7 7) (Castles 0-0 0-7 7-0 #f)]
                     [_ (Castles 0-0 0-7 7-0 7-7)])]
                  [else (Castles 0-0 0-7 7-0 7-7)])]))])])))
(check-expect
 (apply-move
  (ChessGame
   (strings->board
...
;;Additional features
;;This feature allows the user to "regret"
(: apply-move~ : Move ChessGame -> ChessGame)
(define (apply-move~ m g) (apply-move g m))

(: repeat : (Listof Move) -> ChessGame)
(define (repeat moves)
  (foldr apply-move~ new-game moves))

(: take-back-move : ChessGame -> ChessGame)
(define (take-back-move game)
  (match game
    [(ChessGame _ _ history _)
     (repeat (rest history))]))

(provide take-back-move)
;; === graders' tests
...
(check-expect
 (board-update GRADER_starting-board (Loc 0 7) (Some (Piece 'Pawn 'White)))
 (GRADER_board-update GRADER_starting-board (Loc 0 7)
 (Some (Piece 'Pawn 'White))))

(check-expect
 (board-update GRADER_starting-board (Loc 7 0) (Some (Piece 'Rook 'Black)))
 (GRADER_board-update GRADER_starting-board (Loc 7 0)
 (Some (Piece 'Rook 'Black))))

(check-expect
 (board-update GRADER_starting-board (Loc 7 7) (Some (Piece 'Queen 'Black)))
 (GRADER_board-update GRADER_starting-board (Loc 7 7)
 (Some (Piece 'Queen 'Black))))

(check-expect
 (board-update GRADER_starting-board (Loc 4 5) (Some (Piece 'Knight 'Black)))
